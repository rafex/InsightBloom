...
> **Branch**: `main`
> **Creado**: 2026-07-28
> **Último checkpoint**: 2026-07-31 (estados canónicos en encuesta, presentación e IDE)
...
| **Gobierno de componentes** | Estados semánticos | En progreso | `StatusBadge.vue` centraliza estados de usuarios, roles, detalle de usuario, eventos e historial con tonos semánticos reutilizables; la cartelera pública usa su variante `pill` para mantener altura, alineación y forma consistentes |
| **Gobierno de componentes** | Estados vacíos | En progreso | `EmptyState.vue` centraliza mensajes y acciones de ausencia de datos en usuarios, roles, tipos de evento, eventos, dispositivos, reservas y resultados de encuestas |
...
| **Gobierno de componentes** | Estados de carga | En progreso | `LoadingState.vue` unifica la espera de Diagramas, Notas colaborativas, Videollamada, Pizarra, Presentación, IDE, Encuesta y ahora Presentar con semántica `status` y `aria-live`; `FeedbackMessage` reemplaza el error local del IDE |
| **Gobierno de componentes** | Feedback de operaciones | En progreso | `FeedbackMessage.vue` centraliza errores y confirmaciones de Login, Registro, Perfil, acceso a eventos, invitación/control remoto de videollamada, sesión IDE, modo offline del presentador y configuración administrativa con tonos semánticos y roles accesibles |
...
| **Gobierno visual** | Iconografía administrativa | En progreso | `UiIcon.vue` aporta SVGs lineales para métricas del Panel y herramientas del evento; los estados de la cartelera pública reutilizan `StatusBadge` con la variante `pill`; se conservan emojis que forman parte deliberada del contenido editorial |
...
| **Accesibilidad dashboard** | Teclado, labels, foco y responsive | En progreso | `ConferenceConfigPage` usa tabs navegables con flechas/Home/End, panel semántico, controles asociados por `label`/`for` y layout móvil; tablas administrativas exponen acciones y campos inline operables por teclado; `ToggleSwitch` acepta `aria-label`, la moderación de herramientas nombra los interruptores globales e individuales y adapta sus tarjetas a móvil, gestión de boletos apila acciones en pantallas estrechas y moderación IDE adapta toolbar, pods y asientos; configuración, presentación, mapas, nubes, tutor IA y menús compartidos usan foco y navegación por teclado |
...
| **Entrega de presentaciones** | Compresión HTTP | En progreso | Gzip reactivado solo para respuestas textuales proxificadas de `/api/presentations`; falta verificar en producción Firefox/Chromium con presentaciones nuevas y existentes |
