...
| **Gobierno de componentes** | Estados de carga | En progreso | `LoadingState.vue` unifica la espera de Diagramas, Notas colaborativas, Videollamada, Pizarra, Presentación e IDE con semántica `status` y `aria-live`; `FeedbackMessage` reemplaza el error local del IDE |
