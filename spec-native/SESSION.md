...
| **Gobierno de componentes** | Estados de carga | En progreso | `LoadingState.vue` unifica la espera de Diagramas, Notas colaborativas, Videollamada, Pizarra, Presentación, IDE y ahora la carga inicial de Encuesta con semántica `status` y `aria-live`; `FeedbackMessage` reemplaza el error local del IDE |
