...
| **Accesibilidad dashboard** | Teclado, labels, foco y responsive | En progreso | `ConferenceConfigPage` usa tabs navegables con flechas/Home/End, panel semántico, controles asociados por `label`/`for` y layout móvil; tablas administrativas exponen acciones y campos inline operables por teclado; `ToggleSwitch` acepta `aria-label` y la moderación de herramientas nombra los interruptores globales e individuales; configuración, presentación, mapas, nubes, tutor IA y menús compartidos usan foco y navegación por teclado |
