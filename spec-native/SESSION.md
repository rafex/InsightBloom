...
|----------|------------|-------------|
| `in_progress` | No | Completar validación autenticada responsive por rol y verificar el commit desplegado |
...
- [x] `EditConferencePage` — adopta `SaveState`, detecta cambios sobre el formulario completo y evita enviar guardados redundantes.
- [x] `VenueMapEditorPage` — adopta estados independientes para la imagen del recinto y la edición de asientos, evitando guardados redundantes y exponiendo errores de persistencia.
- [x] `WorkspaceFileEditor` — observa cambios del modelo Monaco, distingue contenido pendiente de guardado y usa el estado canónico también para conflictos de escritura.
