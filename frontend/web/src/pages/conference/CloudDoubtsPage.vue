<template lang="pug">
.cloud-page.cloud-page-doubts
  .cloud-header
    h2 Dudas
    span.count(v-if="words.length") {{ words.length }} palabras

  .cloud-submit-box(v-if="canSubmit")
    input.cloud-submit-input(v-model="word" type="text" aria-label="Duda" placeholder="Escribe tu duda en una palabra o frase corta" maxlength="80" @keyup.enter="submit")
    input.cloud-submit-input(v-model="detail" type="text" aria-label="Detalle de la duda" placeholder="Detalle (opcional)" maxlength="240" @keyup.enter="submit")
  ...
  LoadingState(v-if="loading" message="Cargando dudas…")
  EmptyState(v-else-if="!words.length" message="No hay dudas aún. Sé el primero en enviar una.")
  WordCloud(
    v-else
    :words="words"
    :width="cloudWidth"
