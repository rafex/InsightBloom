...
class AssignSandboxUseCaseTest {
    ...
    @Test
    void testSwitchingVariantFreesPreviousSandboxBeforeAssigningNew() {
        // Un alumno tiene un solo sandbox activo a la vez -- pedir la otra variante libera el
        // sandbox actual (Pod + fila) antes de seguir con la asignacion normal de la nueva.
        testConf.setSandboxCliPoolSize(1);
        Mockito.when(conferenceRepoMock.findByUuid("conf-1")).thenReturn(Optional.of(testConf));
        final var existingWeb = new Sandbox("conf-1", 0, "user-student-1", java.time.Instant.now().plusSeconds(3600));
        Mockito.when(sandboxRepoMock.findByConferenceAndUser("conf-1", "user-student-1")).thenReturn(Optional.of(existingWeb));
        ...
        assertNotSame(existingWeb, result);
    ...
    void testAssignSandboxConferenceNotFound() {
        ...
        final var ex = assertThrows(IllegalArgumentException.class,
            () -> useCase.execute("nonexistent", "user-student-1", Sandbox.VARIANT_WEB));
    ...
    void testSharedNeovimPodFillsSeatsBeforeOpeningNewPod() {
        ...
        Mockito.when(sandboxRepoMock.findByConferenceUuid("conf-1")).thenReturn(List.of());
        final var s1 = useCase.execute("conf-1", "user-1", Sandbox.VARIANT_CLI);
        ...
        final var s2 = useCase.execute("conf-1", "user-2", Sandbox.VARIANT_CLI);
        ...
        final var s3 = useCase.execute("conf-1", "user-3", Sandbox.VARIANT_CLI);
        assertEquals(2, s3.getSeatIndex());

        Mockito.when(sandboxRepoMock.findByConferenceUuid("conf-1")).thenReturn(List.of(s1, s2, s3));
        final var s4 = useCase.execute("conf-1", "user-4", Sandbox.VARIANT_CLI);
        ...
        final var s5 = useCase.execute("conf-1", "user-5", Sandbox.VARIANT_CLI);
    ...
    void testSharedNeovimPodFullAcrossAllPodsThrows() {
        testConf.setSandboxSeatsPerPod(4);
        testConf.setSandboxCliPoolSize(2); // capacidad total = 2*4 = 8
        Mockito.when(conferenceRepoMock.findByUuid("conf-1")).thenReturn(Optional.of(testConf));
        Mockito.when(sandboxRepoMock.findByConferenceAndUser(Mockito.eq("conf-1"), Mockito.anyString()))
            .thenReturn(Optional.empty());
        Mockito.when(sandboxRepoMock.findUnassigned("conf-1")).thenReturn(Optional.empty());
        ...
        for (int slot = 0; slot < 2; slot++) {
            for (int seat = 0; seat < 4; seat++) {
                full.add(new Sandbox("conf-1", slot, seat, Sandbox.VARIANT_CLI, "user-" + slot + "-" + seat,
        ...
        final var ex = assertThrows(IllegalArgumentException.class,
            () -> useCase.execute("conf-1", "user-nuevo", Sandbox.VARIANT_CLI));
        assertEquals("sandbox_pool_full", ex.getMessage());
    }
}
