...
public class SqliteSandboxRepository implements SandboxRepository {
    ...
    private static final String SELECT_COLUMNS =
        "uuid, conference_uuid, sandbox_slot, seat_index, variant, user_uuid, assigned_at, created_at, expires_at ";
        ...
        try (final Connection conn = databaseManager.getConnection();
             final PreparedStatement stmt = conn.prepareStatement("""
                INSERT INTO sandbox_assignments (
                    uuid, conference_uuid, sandbox_slot, seat_index, variant, user_uuid, assigned_at,
            ...
            stmt.setString(5, sandbox.getVariant());
            stmt.setString(6, sandbox.getUserUuid());
            stmt.setString(7, sandbox.getAssignedAt() != null ? sandbox.getAssignedAt().toString() : null);
            stmt.setString(8, sandbox.getCreatedAt().toString());
            stmt.setString(9, sandbox.getExpiresAt().toString());
            stmt.executeUpdate();
    ...
    public void deleteByUuid(final String uuid) {
        try (final Connection conn = databaseManager.getConnection();
             final PreparedStatement stmt = conn.prepareStatement(
                "DELETE FROM sandbox_assignments WHERE uuid = ?")) {
            stmt.setString(1, uuid);
            stmt.executeUpdate();
        } catch (final SQLException e) {
            throw new RuntimeException("Failed to delete sandbox by uuid", e);
        }
    }
