...
/** Valida y normaliza avatares antes de guardarlos o publicarlos. */
final class ProfileImageNormalizer {
    ...
    private static final int MAX_DATA_URL_CHARS = 2_100_000;
    private static final int MAX_INPUT_BYTES = 1_500_000;
    ...
    static String normalize(final String dataUrl) {
        ...
        if (trimmed.length() > MAX_DATA_URL_CHARS) throw new IllegalArgumentException("profile_photo_too_large");
        final Matcher matcher = DATA_URL.matcher(trimmed);
        if (!matcher.matches()) throw new IllegalArgumentException("profile_photo_format_not_allowed");
        final byte[] input;
        try {
            input = Base64.getDecoder().decode(matcher.group(2));
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException("profile_photo_invalid_base64");
        }
        if (input.length > MAX_INPUT_BYTES) throw new IllegalArgumentException("profile_photo_too_large");
        try (ImageInputStream imageInput = ImageIO.createImageInputStream(new ByteArrayInputStream(input))) {
            if (imageInput == null) throw new IllegalArgumentException("profile_photo_invalid_image");
            final Iterator<ImageReader> readers = ImageIO.getImageReaders(imageInput);
            if (!readers.hasNext()) throw new IllegalArgumentException("profile_photo_invalid_image");
            ...
            try {
                ...
                if (width <= 0 || height <= 0 || width > MAX_DIMENSION || height > MAX_DIMENSION) {
                    throw new IllegalArgumentException("profile_photo_dimensions_not_allowed");
                }
                final BufferedImage source = reader.read(0);
                if (source == null) throw new IllegalArgumentException("profile_photo_invalid_image");
            ...
            throw new IllegalArgumentException("profile_photo_invalid_image");
        }
    }
