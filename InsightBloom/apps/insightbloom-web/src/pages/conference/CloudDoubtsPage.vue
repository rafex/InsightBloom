...
.cloud-page
  ...
  .cloud-empty(v-if="!loading && !words.length") No hay dudas aún. Los participantes pueden enviar mensajes con /duda
  WordCloud(
    v-if="words.length"
    :words="words"
    :width="cloudWidth"
...
import { getDoubtCloud } from '@/services/api/queryApi'
export default {
  ...
  setup(props) {
    ...
    async function load() {
      ...
      try { words.value = await getDoubtCloud(props.conferenceId) }
